...
function VisionBoard({ session }) {
  ...
  const [isPrivateMission, setIsPrivateMission] = useState(false); // New lock for missions
  const [isPrivateVision, setIsPrivateVision] = useState(false); // New lock for thoughts
  ...
  // ADD MISSION (With Granular Privacy)
  ...
  // CAPTURE VISION (With Granular Privacy)
  const handleCapture = async () => { 
      ...
      try { if (mediaFile) { const ext = mediaFile.name.split('.').pop() || 'mov'; const fileName = `${mediaType}-${timestamp}.${ext}`; const { data, error } = await supabase.storage.from('images').upload(fileName, mediaFile); if (error) throw error; if (data) { const publicUrl = supabase.storage.from('images').getPublicUrl(fileName).data.publicUrl; if (mediaType === 'image') imageUrl = publicUrl; if (mediaType === 'video') videoUrl = publicUrl; } } if (audioBlob) { const ext = audioBlob.type.includes('mp4') ? 'mp4' : 'webm'; const fileName = `audio-${timestamp}.${ext}`; const { data, error } = await supabase.storage.from('images').upload(fileName, audioBlob); if (error) throw error; if (data) audioUrl = supabase.storage.from('images').getPublicUrl(fileName).data.publicUrl; } 
      const { data, error } = await supabase.from('thoughts').insert([{ 
          text: currentInput, image_url: imageUrl, video_url: videoUrl, audio_url: audioUrl, is_quote: isQuoteMode, ignited: false, archived: false, user_id: session.user.id, goal_id: selectedGoalId, is_private: isPrivateVision 
      }]).select(); 
      if (error) throw error; if (data) { setMyThoughts([data[0], ...myThoughts]); calculateStreak([data[0], ...myThoughts]); setCurrentInput(''); clearMedia(); setIsPrivateVision(false); setDebugLog('Relay Secured.'); setTimeout(() => setDebugLog(''), 2000); } } catch (err) { console.error(err); setDebugLog("Error: " + err.message); } finally { setUploading(false); } 
  };
      ...
      <div style={{ maxWidth: '400px', width: '100%', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        {mode === 'morning' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', paddingBottom: '40px' }}>
            ...
            {/* ... (Vision and Ally Tabs remain identical to previous message) ... */}
