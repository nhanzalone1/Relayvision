...
// Using reliable CDN hosted short UI sounds
const SFX = {
  click: new Audio('https://cdn.pixabay.com/audio/2022/03/10/audio_c8c8a73467.mp3'), // Soft click
  success: new Audio('https://cdn.pixabay.com/audio/2021/08/04/audio_0625c153e2.mp3'), // Bell/Check
  crush: new Audio('https://cdn.pixabay.com/audio/2022/03/15/audio_7368828983.mp3'), // Heavy impact
  tab: new Audio('https://cdn.pixabay.com/audio/2022/03/24/audio_720b06b72a.mp3'), // Swoosh
};

// Preload sounds
Object.values(SFX).forEach(sound => { sound.volume = 0.5; sound.load(); });

const playSound = (type) => {
  if (SFX[type]) {
    SFX[type].currentTime = 0;
    SFX[type].play().catch(e => console.log("Audio play blocked (user interaction needed first)"));
  }
};
...
function VisionBoard({ session }) {
  ...
  const [partnerStreak, setPartnerStreak] = useState(0); // New Partner Streak
  const [rank, setRank] = useState('Initiate'); // New Rank State
  ...
  // --- RANK SYSTEM ---
  const getRank = (days) => {
      if (days >= 100) return { title: 'Titan', color: '#c084fc' }; // Purple
      if (days >= 30) return { title: 'Architect', color: '#f59e0b' }; // Gold
      if (days >= 7) return { title: 'Visionary', color: '#3b82f6' }; // Blue
      return { title: 'Initiate', color: '#94a3b8' }; // Grey
  };
  ...
  // Logic Blocks (Send Invite, Goals, etc.) - Preserved
          ...
          <div style={{ position: 'relative' }} ref={menuRef}>
              ...
              <button onClick={() => { setShowProfileMenu(!showProfileMenu); playSound('click'); }} style={{ border: 'none', background: 'transparent', padding: 0, cursor: 'pointer' }}> 
                  <img 
                    ...
                    style={{ width: '42px', height: '42px', borderRadius: '50%', objectFit: 'cover', backgroundColor: currentProfile?.avatar_url ? 'transparent' : 'white', border: currentProfile?.avatar_url ? `2px solid ${mode === 'night' ? '#333' : '#cbd5e1'}` : '2px solid #fbbf24', boxShadow: currentProfile?.avatar_url ? 'none' : '0 0 10px rgba(251, 191, 36, 0.5)' }} 
                  />
              </button>
              {showProfileMenu && (
