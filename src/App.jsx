...
// --- ORGANIC AUDIO ENGINE (Phase 9.4) ---
const SFX = {
  // "Pop": Sharp cork pop (Status: Keep)
  success: new Audio('https://assets.mixkit.co/active_storage/sfx/2003/2003-preview.mp3'),
  
  // "Crush": HEAVY GAS IGNITION. No digital beeps. Just fire.
  crush: new Audio('https://assets.mixkit.co/active_storage/sfx/190/190-preview.mp3'),
  
  // "Fireworks": REAL BARRAGE. Loud, instant explosions.
  fireworks: new Audio('https://assets.mixkit.co/active_storage/sfx/1675/1675-preview.mp3'),
};

// Force Preload & Max Volume
...
const playSound = (type) => {
  ...
  if (audio) {
    ...
    clone.volume = 1.0;
    // Promise handling to ensure browser doesn't block it
    const playPromise = clone.play();
    if (playPromise !== undefined) {
        playPromise.catch(error => {
            console.log("Audio playback failed (usually requires interaction first):", error);
        });
    }
  }
};
...
function VisionBoard({ session }) {
  ...
  // UPDATED: TOGGLE CRUSHED (FAST AUDIO TRIGGER)
  const toggleCrushed = async (mission) => { 
      ...
      if (newCrushed) {
          if (activeCount === 0) playSound('fireworks'); // FINALE
          else playSound('crush'); // FIRE SWOOSH
      }
  ...
  const deleteThought = async (id) => { const { error } = await supabase.from('thoughts').delete().eq('id', id); if (!error) { const newThoughts = myThoughts.filter(t => t.id !== id); setMyThoughts(newThoughts); calculateStreak(newThoughts, setStreak); } };
  ...
  return (
    <div style={mode === 'night' ? nightStyle : morningStyle}>
          ...
          <div style={{ position: 'relative' }} ref={menuRef}>
              ...
              {showProfileMenu && (
                  <div style={{ position: 'absolute', top: '55px', left: 0, background: mode === 'night' ? '#1e293b' : 'white', border: '1px solid #334155', borderRadius: '16px', padding: '8px', width: '200px', boxShadow: '0 10px 25px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                      ...
                      <button onClick={() => avatarInputRef.current.click()} style={{ padding: '12px', display: 'flex', alignItems: 'center', gap: '10px', background: 'transparent', border: 'none', color: mode === 'night' ? 'white' : '#1e293b', fontSize: '14px', cursor: 'pointer', textAlign: 'left', borderRadius: '8px' }}><Upload size={16} /> Upload Photo</button>
                      <button onClick={() => { setMode(mode === 'night' ? 'morning' : 'night'); setShowProfileMenu(false); }} style={{ padding: '12px', display: 'flex', alignItems: 'center', gap: '10px', background: 'transparent', border: 'none', color: mode === 'night' ? 'white' : '#1e293b', fontSize: '14px', cursor: 'pointer', textAlign: 'left', borderRadius: '8px' }}>{mode === 'night' ? <><Sun size={16} /> Morning Mode</> : <><Moon size={16} /> Night Mode</>}</button>
      ...
      <div style={{ maxWidth: '400px', width: '100%', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ marginTop: '100px', textAlign: mode === 'night' ? 'center' : 'left' }}>
          {mode === 'night' ? ( <> 
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '10px' }}> 
                ...
                <div onClick={() => setHistoryModal(true)} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#fff7ed', padding: '6px 12px', borderRadius: '20px', border: '1px solid #ffedd5', cursor: 'pointer' }}>
        ...
        {/* ... (Night Mode - Same as before) ... */}
        {mode === 'night' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
             {/* ... (Night Mode Goals, Input, Vision, Ally, etc.) ... */}
